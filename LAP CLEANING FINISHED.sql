...
SELECT * 
FROM LAPT2
WHERE BRAND IN ("APPLE","HP")
ORDER BY BRAND ASC;

SELECT * 
FROM LAPT2
WHERE LAP_TYPE LIKE "ULTRA%";

SELECT *
FROM LAPT2
WHERE LAP_TYPE LIKE "ULTRA%" AND SCREEN_RESOLUTION LIKE "%FULL HD%";

SELECT *
FROM LAPT2
WHERE LAP_TYPE LIKE "ULTRA%" 
HAVING SCREEN_RESOLUTION LIKE "%FULL HD%";
...
SELECT *
FROM LAPT2 
WHERE MEMORY_SIZE_GB > 512;

SELECT BRAND,COUNT(LAP_ID)
FROM LAPT2
GROUP BY BRAND
HAVING COUNT(LAP_ID)>5
ORDER BY BRAND ASC;

SELECT BRAND,LAP_TYPE,MIN(PRICE),MAX(PRICE),ROUND(AVG(PRICE),2)
FROM LAPT2
GROUP BY BRAND,LAP_TYPE
ORDER BY BRAND ASC;

SELECT *,RANK() OVER(PARTITION BY BRAND ORDER BY PRICE DESC)
FROM LAPT2;

SELECT *,RANK() OVER(PARTITION BY BRAND ORDER BY INCHES DESC)
FROM LAPT2;
